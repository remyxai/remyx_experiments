...
# Modified example from depthai-experiments:
# https://github.com/luxonis/depthai-experiments/blob/master/gen2-tf-image-classification/main.py
# 
# Run:
# python main.py -cam -m model/
# python main.py -vid <path_to_video> -m model/
...
parser.add_argument("-m","--model_dir",type=str,help="Directory of the model assets",default="model/")
...
with open(str(Path(args.model_dir, "metadata.csv")), "r") as metadata_file:
...
detection_nn.setBlobPath(str(Path(args.model_dir, f"{metadata['name']}.blob")))
...
if camera:
    ...
    manip.initialConfig.setResize(metadata["input_shape"][1], metadata["input_shape"][2])
...
with dai.Device(pipeline) as device:
    ...
    class_names = metadata["labels"].split("|")
    result = None
    ...
    while should_run():
        ...
        if not camera:
            nn_data = dai.NNData()
            nn_data.setLayer("input", to_planar(frame, (metadata["input_shape"][1], metadata["input_shape"][2])))
        ...
        if debug:
            ...
            if result is not None:
                cv2.putText(frame, "{}".format(result["name"]), (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 0), thickness=2, lineType=cv2.LINE_AA)
