...
void RespondToDetection(float rfid_score, float iphone_score) {
  int rfid_score_int = (rfid_score) * 100 + 0.5;
  (void) iphone_score; // unused
#if DISPLAY_SUPPORT
#if 1 // USE LVGL
    ...
    if (rfid_score_int < 60) { // treat score less than 60% as iphone
      color = 0x3f; // green
    }
...
#endif
#endif
  MicroPrintf("rfid score:%d%%, iphone score %d%%",
              rfid_score_int, 100 - rfid_score_int);
}
